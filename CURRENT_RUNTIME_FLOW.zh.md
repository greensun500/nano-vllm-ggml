...
full-attention 层在 math/flash 路径先批量写 K/V，再按 `read_slots[C]` gather，chunk 使用 `[C,T]` causal mask 防止 query 看到未来 token。`native_attention_impl` 默认 `auto`：非 MTP 按真实 shape probe `FLASH_ATTN_EXT`，成功后把 QK -> softmax -> PV 留在 GGML backend kernel 内，失败则回退 math；显式 `flash` 在不支持时报告清晰错误。v3.89 起 native Vulkan 的 MTP `auto` 也逐张 probe：T=1 draft 和 T=K+1 verification 都支持时分别用 Flash，CPU/CUDA 或不支持的 shape 仍为 math。Arm/Mali real-model oracle 已比较 math、flash、auto 的 full/partial/reject trace；不要把这个已验证范围泛化到其他 GPU 或 GGML revision。
...
v3.92 重新以完整 real-model long oracle 验证 T>1 direct kernel：MTP-off/on、
`max_num_batched_tokens=64/768`、verification KV fusion 的 greedy trace 都与 math 一致，
因此显式 `paged` 可用于 multi-token prefill 和 MTP verification。`paged` 仍不参加
`auto`，所以不会改变 CPU/CUDA、非 MTP 默认或 MTP acceptance 基线；不符合固定
F32/256-dim/GQA contract 的 shape 仍拒绝，而不是静默改写 attention 语义。
...
- MTP 的串行 draft、tied vocab head 扫描、host acceptance/readback 和多份 GDN snapshot 仍是主要成本。v3.77 bench 会将 native draft / target verification / KV catch-up 的累计 wall 与 setup time 分开导出；Mali K=1 math 为 `12.377 / 68.134 / 0.784s`，故 verification 而非图 setup（约 `1.7s`）是主导成本。v3.88 把 verification 后的 KV-only graph 合入 verification graph，消除了 KV-only 计时但只有约 `0.87%` decode 收益。v3.89 在当前 Arm/Mali GGML revision 重新以 real-model oracle 验证 MTP Flash：auto 的 draft/verification/KV 变为 `12.191 / 56.656 / 0.765s`，decode `19.885 -> 23.219 tok/s`（+16.76%）、acceptance 仍为 `100%`，因此它成为这个受 probe 保护的默认路径。186-token 的 T=2 verification kernel 内，Q6_K head + argmax 仍是后续融合目标；当前实验 Q6_K fused head 因行级并行度下降保持 opt-in。4096-token math verification 中 GET_ROWS 增至 `5.25ms`，但 score/value matmul 仍约 `62.7 / 134.5ms`；因此 direct paged attention 必须做 page-table 直读和 online softmax/value accumulation，而非仅消除 gather。Mali K=1 的 experimental persistent-graph A/B 还显示高 cache hit 不能抵消 padded bucket 与动态输入开销，故默认继续 eager。
