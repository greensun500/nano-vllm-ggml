...
- MTP 的串行 draft、tied vocab head 扫描、host acceptance/readback 和多份 GDN snapshot 仍是主要成本。v3.77 bench 会将 native draft / target verification / KV catch-up 的累计 wall 与 setup time 分开导出；固定 high-performance Vulkan build 的长 decode 中，K=1/2/3 都没有超过 MTP-off；K=1 的小 verification 窗口最慢。Mali K=1 的 experimental persistent-graph A/B 还显示高 cache hit 不能抵消 padded bucket 与动态输入开销，故默认继续 eager。
