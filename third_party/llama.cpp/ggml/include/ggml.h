...
extern "C" {
    ...
    enum ggml_op {
        ...
        // Attention over a persistent KV cache addressed through an I32 slot
        // table. Callers must select it only after verifying backend support.
        GGML_OP_PAGED_ATTN,
    ...
    // q is [head_dim, n_heads, n_tokens], K/V cache are
    // [head_dim * n_kv_heads, physical_slots], and slots is I32 [n_kv].
    // The output is F32 [head_dim * n_heads, n_tokens]. The operation applies
    // the standard causal prefix mask for a contiguous query chunk.
