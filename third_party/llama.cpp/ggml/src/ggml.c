...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "glu(x)",
    "paged_attn(q,k,v,slots,context_len)",
};
...
struct ggml_tensor * ggml_paged_attn(
        struct ggml_context * ctx,
        struct ggml_tensor  * q,
        struct ggml_tensor  * key_cache,
        struct ggml_tensor  * value_cache,
        struct ggml_tensor  * slots,
        struct ggml_tensor  * context_len,
        float                 scale) {
    ...
    GGML_ASSERT(key_cache->ne[0] == value_cache->ne[0]);
    GGML_ASSERT(key_cache->ne[1] == value_cache->ne[1]);
    GGML_ASSERT(slots->ne[0] > 0);
    GGML_ASSERT(ggml_nelements(context_len) == 1);
    GGML_ASSERT(key_cache->ne[0] % q->ne[0] == 0);
    GGML_ASSERT(q->ne[1] % (key_cache->ne[0] / q->ne[0]) == 0);

    const int64_t ne[2] = { q->ne[0] * q->ne[1], q->ne[2] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 2, ne);
    ggml_set_op_params_f32(result, 0, scale);
    result->op = GGML_OP_PAGED_ATTN;
    result->src[0] = q;
    result->src[1] = key_cache;
    result->src[2] = value_cache;
    result->src[3] = slots;
    result->src[4] = context_len;
    return result;
}
