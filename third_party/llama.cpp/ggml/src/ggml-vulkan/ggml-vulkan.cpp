...
struct vk_device_struct {
    ...
    vk_pipeline pipeline_paged_attn_f32;
    vk_pipeline pipeline_mul_mat_vec_q6_k_argmax;
    vk_pipeline pipeline_q6_k_argmax_reduce;
...
struct vk_lm_head_argmax_push_constants {
    uint32_t ncols;
    uint32_t nrows;
    uint32_t ntokens;
...
struct vk_lm_head_argmax_reduce_push_constants {
...
struct ggml_backend_vk_context {
    ...
    // A validated Q6_K vocabulary MUL_MAT followed directly by ARGMAX can
    // bypass the materialized logits tensor.  This flag is reset per graph
    // node and is intentionally not a general MUL_MAT fusion mode.
    bool fused_lm_head_argmax {};
...
#undef CREATE_GLU
    ...
    // Qwen3.5's tied Q6_K vocabulary head has 248k rows.  The generic
    // argmax shader used one subgroup (16 lanes on Mali-G720), leaving each
    // lane to scan more than 15k logits.  A 256-lane workgroup is supported
    // by the Arm devices targeted by this runtime and keeps the same stable
    // lowest-index-on-tie reduction semantics while exposing more memory
    // parallelism.  Keep the upstream subgroup-sized specialization for all
    // other vendors until it has separate device evidence.
    const uint32_t argmax_block_size = device->vendor_id == VK_VENDOR_ID_ARM
        ? (1u << std::min(8u, device->max_workgroup_size_log2))
    ...
    ggml_vk_create_pipeline(device, device->pipeline_paged_attn_f32,
        "paged_attn_f32", paged_attn_f32_len, paged_attn_f32_data, "main", 6,
        sizeof(vk_paged_attn_push_constants), { 256, 1, 1 }, { 256 }, 1);
    ggml_vk_create_pipeline(device, device->pipeline_mul_mat_vec_q6_k_argmax,
        "mul_mat_vec_q6_k_argmax", mul_mat_vec_q6_k_argmax_len,
        mul_mat_vec_q6_k_argmax_data, "main", 3,
        sizeof(vk_lm_head_argmax_push_constants), { 16, 1, 1 }, { 16 }, 1);
    ggml_vk_create_pipeline(device, device->pipeline_q6_k_argmax_reduce,
        "q6_k_argmax_reduce", q6_k_argmax_reduce_len,
        q6_k_argmax_reduce_data, "main", 2,
        sizeof(vk_lm_head_argmax_reduce_push_constants), { 256, 1, 1 }, { 256 }, 1);
...
static bool ggml_vk_should_use_mmvq(const vk_device& device, uint32_t m, uint32_t n, uint32_t k, ggml_type src0_type) {
    ...
    // Mali integer-dot MMVQ wins for multi-column Q4_0 verification, but the
    // activation quantization and extra dispatch do not amortize for scalar
    // decode. Keep the explicit FORCE/DISABLE modes above as diagnostic
    // overrides and specialize only the measured default path.
    if (device->vendor_id == VK_VENDOR_ID_ARM &&
...
static void ggml_vk_mul_mat_q6_k_argmax(
        ggml_backend_vk_context * ctx,
        vk_context & subctx,
        const struct ggml_cgraph * cgraph,
        int node_idx) {
    const ggml_tensor * logits = cgraph->nodes[node_idx];
    const ggml_tensor * weights = logits->src[0];
    const ggml_tensor * hidden = logits->src[1];
    ggml_tensor * tokens = cgraph->nodes[node_idx + 1];

    GGML_ASSERT(weights->type == GGML_TYPE_Q6_K);
    GGML_ASSERT(weights->ne[0] == hidden->ne[0]);
    GGML_ASSERT(ggml_is_contiguous(weights));
    GGML_ASSERT(ggml_is_contiguous(hidden));
    GGML_ASSERT(tokens->op == GGML_OP_ARGMAX);
    GGML_ASSERT(tokens->src[0] == logits);
    GGML_ASSERT(tokens->type == GGML_TYPE_I32);

    constexpr uint32_t kRowsPerCandidate = 16;
    const uint32_t candidate_count = CEIL_DIV((uint32_t) weights->ne[1], kRowsPerCandidate);
    const uint32_t token_count = (uint32_t) hidden->ne[1];
    const size_t candidate_bytes = (size_t) candidate_count * token_count * 2 * sizeof(uint32_t);

    // This is transient within the fused pair.  Unlike the old logits tensor,
    // it is not part of the graph allocator's lifetime and is 16x smaller.
    if (ctx->prealloc_size_split_k < candidate_bytes) {
        ctx->prealloc_size_split_k = candidate_bytes;
        ggml_vk_preallocate_buffers(ctx, subctx);
    }
    GGML_ASSERT(ctx->prealloc_split_k != nullptr);
    if (ctx->prealloc_split_k_need_sync) {
        ggml_vk_sync_buffers(ctx, subctx);
    }

    const vk_subbuffer weights_buf = ggml_vk_tensor_subbuffer(ctx, weights);
    const vk_subbuffer hidden_buf = ggml_vk_tensor_subbuffer(ctx, hidden);
    const vk_subbuffer candidates_buf = ggml_vk_subbuffer(ctx, ctx->prealloc_split_k, 0);
    const vk_subbuffer tokens_buf = ggml_vk_tensor_subbuffer(ctx, tokens);

    ggml_pipeline_request_descriptor_sets(ctx, ctx->device->pipeline_mul_mat_vec_q6_k_argmax, 1);
    ggml_pipeline_request_descriptor_sets(ctx, ctx->device->pipeline_q6_k_argmax_reduce, 1);
    ggml_vk_dispatch_pipeline(ctx, subctx, ctx->device->pipeline_mul_mat_vec_q6_k_argmax,
        { weights_buf, hidden_buf, candidates_buf },
        vk_lm_head_argmax_push_constants{
            (uint32_t) weights->ne[0], (uint32_t) weights->ne[1], token_count,
            candidate_count },
        { candidate_count * kRowsPerCandidate, token_count, 1 });
    ggml_vk_sync_buffers(ctx, subctx);
    ggml_vk_dispatch_pipeline(ctx, subctx, ctx->device->pipeline_q6_k_argmax_reduce,
        { candidates_buf, tokens_buf },
        vk_lm_head_argmax_reduce_push_constants{ candidate_count, candidate_count },
        { token_count * 256, 1, 1 });
    ctx->prealloc_split_k_need_sync = true;
}
...
static void ggml_vk_mul_mat(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    if (ctx->fused_lm_head_argmax) {
        ggml_vk_mul_mat_q6_k_argmax(ctx, subctx, cgraph, node_idx);
        return;
    }
...
static void ggml_vk_paged_attn(
        ggml_backend_vk_context * ctx,
        vk_context & subctx,
        const ggml_tensor * q,
        const ggml_tensor * key_cache,
        const ggml_tensor * value_cache,
        const ggml_tensor * slots,
        const ggml_tensor * context_len,
        ggml_tensor * dst) {
    GGML_ASSERT(q->type == GGML_TYPE_F32);
    GGML_ASSERT(key_cache->type == GGML_TYPE_F32);
    GGML_ASSERT(value_cache->type == GGML_TYPE_F32);
    GGML_ASSERT(slots->type == GGML_TYPE_I32);
    GGML_ASSERT(context_len->type == GGML_TYPE_I32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous(q));
    GGML_ASSERT(ggml_is_contiguous(key_cache));
    GGML_ASSERT(ggml_is_contiguous(value_cache));
    GGML_ASSERT(ggml_is_contiguous(slots));
    GGML_ASSERT(ggml_is_contiguous(context_len));
    GGML_ASSERT(ggml_is_contiguous(dst));
    ...
    const uint32_t n_kv = (uint32_t)slots->ne[0];
    const uint32_t cache_width = (uint32_t)key_cache->ne[0];
    GGML_ASSERT(head_dim == kHeadDim);
    GGML_ASSERT(n_tokens > 0 && n_kv >= n_tokens);
    GGML_ASSERT(ggml_nelements(context_len) == 1);
    GGML_ASSERT(cache_width == (uint32_t)value_cache->ne[0]);
    GGML_ASSERT(cache_width % head_dim == 0);
    const uint32_t n_kv_heads = cache_width / head_dim;
    GGML_ASSERT(n_heads % n_kv_heads == 0);
    GGML_ASSERT(dst->ne[0] == (int64_t)(head_dim * n_heads));
    GGML_ASSERT(dst->ne[1] == n_tokens);

    float scale = 1.0f;
    memcpy(&scale, dst->op_params, sizeof(scale));
    const vk_paged_attn_push_constants pc = {
        n_kv, n_tokens, n_heads, n_kv_heads, cache_width, scale,
    };
    vk_pipeline pipeline = ctx->device->pipeline_paged_attn_f32;
    ...
    ggml_vk_dispatch_pipeline(
        ...
        pipeline,
        {
            ...
            ggml_vk_tensor_subbuffer(ctx, key_cache),
            ggml_vk_tensor_subbuffer(ctx, value_cache),
            ggml_vk_tensor_subbuffer(ctx, slots),
            ggml_vk_tensor_subbuffer(ctx, context_len),
            ggml_vk_tensor_subbuffer(ctx, dst),
        },
        pc,
        { 256, n_heads, n_tokens });
}
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_PAGED_ATTN:
        if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 &&
            src2->type == GGML_TYPE_F32 && dst->src[3] != nullptr &&
            dst->src[3]->type == GGML_TYPE_I32 &&
            dst->src[4] != nullptr && dst->src[4]->type == GGML_TYPE_I32 &&
            dst->type == GGML_TYPE_F32) {
            return ctx->device->pipeline_paged_attn_f32;
        }
        return nullptr;
    case GGML_OP_ACC:
...
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int node_idx, ggml_tensor *node_begin, int node_idx_begin, bool last_node, bool almost_ready, bool submit){
    ...
    case GGML_OP_GET_ROWS_BACK:
        ...
        break;
    case GGML_OP_PAGED_ATTN:
        ggml_vk_paged_attn(ctx, compute_ctx, src0, src1, src2, src3, node->src[4], node);
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        ctx->fused_lm_head_argmax = false;
        ...
        if (!ctx->device->disable_fusion) {
            ...
            if (num_adds) {
                ...
                std::fill_n(op_srcs_fused_elementwise, ctx->num_additional_fused_ops + 1, true);
                       ...
                       cgraph->nodes[i + 1]->op == GGML_OP_ARGMAX &&
                       cgraph->nodes[i + 1]->src[0] == cgraph->nodes[i] &&
                       ctx->device->vendor_id == VK_VENDOR_ID_ARM &&
                       std::getenv("GGML_VK_ENABLE_Q6_K_LM_HEAD_ARGMAX_FUSION") != nullptr &&
                       std::getenv("GGML_VK_DISABLE_Q6_K_LM_HEAD_ARGMAX_FUSION") == nullptr &&
                       cgraph->nodes[i]->src[0]->type == GGML_TYPE_Q6_K &&
                       cgraph->nodes[i]->src[1]->type == GGML_TYPE_F32 &&
                       cgraph->nodes[i]->src[1]->ne[1] <= 4 &&
                       cgraph->nodes[i]->src[1]->ne[2] == 1 &&
                       cgraph->nodes[i]->src[1]->ne[3] == 1 &&
                       ggml_is_contiguous(cgraph->nodes[i]->src[0]) &&
                       ggml_is_contiguous(cgraph->nodes[i]->src[1])) {
                // Do not use ggml_can_fuse() here: its generic sequential
                // fusion contract requires every node to have the same shape,
                // whereas this reduction intentionally changes [vocab, T]
                // logits into [T] token ids.  The direct producer edge above
                // establishes the only intermediate this fusion elides.
                // The fusion is deliberately narrow: a small F32 token block
                // feeding a Q6_K vocabulary head.  Other MUL_MAT+ARGMAX pairs
                // retain their independent materialized-logits semantics.
                ctx->num_additional_fused_ops = 1;
                ctx->fused_lm_head_argmax = true;
                fusion_string = "Q6_K_LM_HEAD_ARGMAX";
...
static bool ggml_backend_vk_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_PAGED_ATTN:
            {
                // The native Qwen3.5 direct-cache kernel deliberately has a
                // narrow contract.  Keep it explicit rather than claiming a
                // generic paged-attention implementation for arbitrary GGML
                // graphs or cache layouts.
                const ggml_tensor * q = op->src[0];
                const ggml_tensor * key_cache = op->src[1];
                const ggml_tensor * value_cache = op->src[2];
                const ggml_tensor * slots = op->src[3];
                if (q == nullptr || key_cache == nullptr || value_cache == nullptr || slots == nullptr) {
                    return false;
                }
                if (q->type != GGML_TYPE_F32 || key_cache->type != GGML_TYPE_F32 ||
                    value_cache->type != GGML_TYPE_F32 || slots->type != GGML_TYPE_I32 ||
                    op->type != GGML_TYPE_F32) {
                    return false;
                }
                if (!ggml_is_contiguous(q) || !ggml_is_contiguous(key_cache) ||
                    !ggml_is_contiguous(value_cache) || !ggml_is_contiguous(slots) ||
                ...
                const uint32_t n_tokens = (uint32_t)q->ne[2];
                const uint32_t cache_width = (uint32_t)key_cache->ne[0];
                if (head_dim != 256 || n_heads == 0 || n_tokens == 0 ||
                    slots->ne[0] < n_tokens || cache_width == 0 ||
                    cache_width != value_cache->ne[0] || cache_width % head_dim != 0) {
                    return false;
                }
                const uint32_t n_kv_heads = cache_width / head_dim;
                return n_kv_heads > 0 && n_heads % n_kv_heads == 0 &&
                    op->ne[0] == (int64_t)(head_dim * n_heads) && op->ne[1] == n_tokens;
            }
        case GGML_OP_GET_ROWS:
            {
